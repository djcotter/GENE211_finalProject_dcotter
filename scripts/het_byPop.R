...
option_list = list(
  ...
  make_option(c('--height'), type='double', default=6.0,
...
p1 <- ggplot(data, aes(x=reorder(subpop,value), y=value, fill=pop)) + 
  geom_errorbar(aes(ymin=val_low, ymax=val_high), color='black') + 
  geom_col() + coord_flip() + theme_pubr() + xlab('Population') +
  ylab('Average Heterozygosity') + scale_fill_manual(stringr::str_wrap('Super Population', width=10), 
                                                   breaks=c('AFR', 'EUR', 'SAS', 'EAS', 'AMR'),
                                                   values=cbbPalette) +
